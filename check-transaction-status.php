...
$transaction_sql = "select * from bbn_transaction where status = 'Waiting' and currency = 'ETH'";
...
while ($transaction = mysqli_fetch_array($transaction_result)) {
	if ((time() - strtotime($transaction['date'])) > 48 * 3600) {
		$reject_transaction_sql = "update bbn_transaction set status = 'Canceled' where transaction_id = '".$transaction['transaction_id']."'";
